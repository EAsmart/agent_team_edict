...
"""
import json, pathlib, subprocess, sys, threading, argparse, datetime, logging, re, os, socket, shutil, shlex
...
def handle_runtime_shell(body):
    """第五阶段 Tool Runtime 本地执行桥：只运行白名单命令，并限制在任务工作目录内。"""
    command = str(body.get('command', '')).strip()
    task_id = str(body.get('taskId', '')).strip() or 'runtime-task'
    if not command:
        return {'ok': False, 'error': 'command required'}
    if not _SAFE_NAME_RE.match(task_id):
        return {'ok': False, 'error': 'taskId 含非法字符'}

    normalized = command.lower()
    blocked = [
        'rm ', 'del ', 'rmdir', 'format', 'shutdown', 'restart-computer',
        'remove-item', 'git reset', 'git checkout --', 'taskkill',
    ]
    for fragment in blocked:
        if fragment in normalized:
            return {'ok': False, 'error': f'命中危险命令片段：{fragment}'}
    ...
    if not args:
        return {'ok': False, 'error': '空命令已被拒绝'}

    executable = pathlib.Path(args[0]).name.lower()
    allowed = {'codex', 'npm', 'node', 'python', 'git', 'rg', 'echo'}
    ...
    # 前端使用虚拟工作区路径，后端执行前映射到本机受限目录。
    virtual_prefix = f'/virtual-court-workspace/{task_id}'
    args = [str(workspace) if arg == virtual_prefix else arg.replace(virtual_prefix, str(workspace)) for arg in args]
    ...
    try:
        ...
        return {'ok': False, 'command': command, 'cwd': str(workspace), 'stdout': '', 'stderr': f'{executable} 未安装或不在 PATH', 'exitCode': 127}
